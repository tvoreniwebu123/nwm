...
if(cookieBanner && localStorage.getItem('jiskra-cookie-info')!=='accepted') showCookieBanner();
document.querySelectorAll('[data-cookie-accept]').forEach(button=>button.addEventListener('click',()=>{localStorage.setItem('jiskra-cookie-info','accepted');hideCookieBanner();}));
document.querySelectorAll('[data-cookie-settings]').forEach(button=>button.addEventListener('click',()=>{localStorage.removeItem('jiskra-cookie-info');showCookieBanner();}));

/* Jazykové mutace CZ / DE / EN */
const translations = {
  de: {
    'Ochrana osobních údajů':'Datenschutz','Nastavení cookies':'Cookie-Einstellungen','Více informací':'Mehr Informationen','Rozumím':'Verstanden','Web používá pouze technické cookies nutné pro uložení jazyka a základní fungování.':'Die Website verwendet nur technisch notwendige Cookies zur Speicherung der Sprache und für grundlegende Funktionen.','Další tým':'Weitere Mannschaft','VIAGEM Ústí nad Labem':'VIAGEM Ústí nad Labem','4. česká fotbalová liga, skupina B.':'4. tschechische Fußballliga, Gruppe B.','Více informací →':'Mehr Informationen →','Soutěž':'Wettbewerb','Skupina':'Gruppe','Budou doplněny později.':'Wird später ergänzt.','Úvod':'Startseite','Soustředění':'Trainingslager','Týmy':'Mannschaften','Sportovní areál':'Sportanlage','Klub':'Verein','O klubu':'Über den Verein','A tým':'A-Mannschaft','Zápasy':'Spiele','Další týmy':'Weitere Mannschaften','Kontakt':'Kontakt','Partneři':'Partner','Rezervace':'Reservierung','Fotogalerie':'Fotogalerie',
  ...
  en: {
    'Ochrana osobních údajů':'Privacy policy','Nastavení cookies':'Cookie settings','Více informací':'More information','Rozumím':'Understood','Web používá pouze technické cookies nutné pro uložení jazyka a základní fungování.':'The website uses only technical cookies required to store the selected language and provide basic functionality.','Další tým':'Other team','VIAGEM Ústí nad Labem':'VIAGEM Ústí nad Labem','4. česká fotbalová liga, skupina B.':'4th Czech Football League, Group B.','Více informací →':'More information →','Soutěž':'Competition','Skupina':'Group','Budou doplněny později.':'Will be added later.','Úvod':'Home','Soustředění':'Training camp','Týmy':'Teams','Sportovní areál':'Sports venue','Klub':'Club','O klubu':'About the club','A tým':'First team','Zápasy':'Matches','Další týmy':'Other teams','Kontakt':'Contact','Partneři':'Partners','Rezervace':'Booking','Fotogalerie':'Gallery',
